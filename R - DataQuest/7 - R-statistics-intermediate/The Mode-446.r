...
mode <- compute_mode(houses$`Roof Style`)$mode
value_counts  <-  compute_mode(houses$`Roof Style`)$values
...
ggplot(data = houses) +
    ...
    xlim(1,12)+
    scale_colour_manual(values = c("green","black", "orange"), 
                        name = "", 
                        labels = c("Mode","Median", "Mean")) +
    theme_bw() + 
